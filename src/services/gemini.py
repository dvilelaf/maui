...
class GeminiService(LLMProvider):
    ...
    def _verify_and_sort_keys(self):
        ...
        for key in self.api_keys:
            try:
                ...
                model = genai.GenerativeModel(
                    "gemini-2.5-flash"
                )  # Use a common model for verification
                # Lightweight check
                model.generate_content("ping")
                working_keys.append(key)
            except Exception as e:
                self.logger.warning(
                    f"Key verification failed for a key (moving to end): {e}"
                )
        ...
        if not working_keys and not exhausted_keys:
            ...
            self.api_keys = (
                exhausted_keys  # Keep all just in case, but will likely fail
            )
        else:
            self.logger.info(
                f"Key optimization complete: {len(working_keys)} working, {len(exhausted_keys)} exhausted."
            )
    ...
    def _get_system_prompt(self) -> str:
        ...
        **Language Support:**
        ...
        - 'formatted_task':
            ...
            - 'recurrence':
              - If user implies repetition: "DAILY", "WEEKLY", "MONTHLY", "YEARLY".
              - "Every day" -> "DAILY".
              - "Every Monday" -> "WEEKLY".
              - "Every month" -> "MONTHLY".
              - If no repetition, null.
    ...
    def process_input(
        ...
        for model_name in models_to_try:
            ...
            if time.time() < cooldown_expiry:
                self.logger.info(
                    f"Skipping model {model_name} due to cooldown (expires in {int(cooldown_expiry - time.time())}s)"
                )
                continue
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if mime_type.startswith("audio/"):
                        prompt_parts.append(
                            {"mime_type": mime_type, "data": user_input}
                        )
                        prompt_parts.append(
                            "Please transcribe this audio and extract the task details."
                        )
                    else:
                    ...
                    self.logger.warning(
                        f"Gemini API error (model {model_name}, attempt {attempt + 1}/{max_retries}): {e}"
                    )
                    last_error = e
                    ...
                    self.logger.warning(
                        f"Gemini Quota Exceeded (KeyIdx: {self.current_key_index}, Model: {model_name}): {e}"
                    )
                    ...
                    while keys_tried < len(self.api_keys) - 1:  # Try other keys
                        ...
                        try:
                            ...
                            response = self.model.generate_content(prompt_parts)
                            self.logger.info(
                                f"Gemini Raw Response (Recovered with Key {self.current_key_index}): {response.text}"
                            )
                            return TaskExtractionResponse.model_validate_json(
                                response.text
                            )
                        except ResourceExhausted:
                            self.logger.warning(
                                f"Key {self.current_key_index} also exhausted."
                            )
                            continue
                        except Exception as inner_e:
                            self.logger.error(
                                f"Error with rotated key {self.current_key_index}: {inner_e}"
                            )
                    ...
                    self.logger.error(
                        f"Error processing input with Gemini ({model_name}): {e}"
                    )
                    last_error = e
                    break
