...
def test_llm_factory_default(mocker):
    ...
    mocker.patch.object(Config, "LLM_PROVIDER", "gemini")
    mocker.patch.object(Config, "GEMINI_API_KEYS", ["fake_key"])
...
def test_llm_factory_groq(mocker):
    # Mock config to groq
    mocker.patch.object(Config, "LLM_PROVIDER", "groq")
...
def test_llm_factory_invalid(mocker):
    mocker.patch.object(Config, "LLM_PROVIDER", "invalid_provider")
